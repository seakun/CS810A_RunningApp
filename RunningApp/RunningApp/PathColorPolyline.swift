//
//  PathColorPolyline.swift
//  RunningApp
//
//  Created by lyu on 16/11/10.
...
class pathColorPolyline: MKPolyline {
    
    var color: UIColor?
    
    
    class func colorSegments(forLocations locations: [Location]) -> [pathColorPolyline] {
        var colorSegments = [pathColorPolyline]()
        ...
        let (speeds, minSpeed, maxSpeed) = allSpeed(forLocations: locations)
        ...
        for i in 1..<locations.count {
            ...
            coords.append(CLLocationCoordinate2D(latitude: l1.lattitude, longitude: l1.longitude))
            coords.append(CLLocationCoordinate2D(latitude: l2.lattitude, longitude: l2.longitude))
            ...
            let segment = pathColorPolyline(coordinates: &coords, count: coords.count)
            segment.color = color
    ...
    private class func allSpeed(forLocations locations: [Location]) -> (speeds: [Double], minSpeed: Double, maxSpeed: Double) {
        ...
        for i in 1..<locations.count {
            ...
            let cl1 = CLLocation(latitude: l1.lattitude, longitude: l1.longitude)
            let cl2 = CLLocation(latitude: l2.lattitude, longitude: l2.longitude)
